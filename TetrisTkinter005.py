        ...
        for j, column in enumerate(row):
            if column == '0':
                canvas.create_rectangle(sx + j*30,  sy + i*30, sx + (j+1)*30, sy + (
                    i+1)*30, fill=color(grid[i][j]), outline="gray", width=1)
...
def draw_window(win):
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            canvas.create_rectangle(top_left_x + j * cell_size, top_left_y + i * cell_size, top_left_x + (
                j+1) * cell_size, top_left_y + (i+1) * cell_size, fill=color(grid[i][j]), outline="gray", width=1)
